...
import {createQuestion, getQuestion, listQuestions, moveDownQuestion} from '../../test-data-seeder/question-data';
import { QuestionType } from '../../constants/test-data-constants';
test.describe('PATCH /api/questionnaires/{questionnaireId}/questions/{questionId}?action=MoveDown', () => {
    test('moves a middle question down by one (204) and orders reflect change', async ({ request }) => {
        ...
        expect(byId[q3.id].order).toBe(2);
        expect(byId[q2.id].order).toBe(3);
    });
    ...
    test('invalid questionnaire id format returns 404', async ({ request }) => {
        const res = await moveDownQuestion(request, 'not-a-guid', 'not-a-guid');
    ...
    test('non-existent ids return 404', async ({ request }) => {
        ...
        const res = await moveDownQuestion(request, questionnaire.id, missing);
        expect(res.status()).toBe(404);
    });
